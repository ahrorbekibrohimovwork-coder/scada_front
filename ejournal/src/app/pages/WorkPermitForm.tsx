...
export function WorkPermitForm() {
  ...
  const fetchStaff = async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        setOfficials({
          issuer:              users.filter((u: any) => u.role === 'issuer').map(mapUser),
          dispatcher:          users.filter((u: any) => u.role === 'dispatcher').map(mapUser),
          dispatcherAssistant: users.filter((u: any) => u.role === 'dispatcher_assistant').map(mapUser),
          admitter:            users.filter((u: any) => u.role === 'admitter').map(mapUser),
          manager:             users.filter((u: any) => u.role === 'manager').map(mapUser),
          observer:            users.filter((u: any) => u.role === 'observer').map(mapUser),
          foreman:             users.filter((u: any) => u.role === 'foreman').map(mapUser),
          worker:              users.filter((u: any) => u.role === 'worker').map(mapUser),
        });
      }
    } catch (e) {
  ...
  const handlePrepareSign = async () => {
    if (!validate()) {
      // Navigate to the section that has the first error
      ...
      const errKeys = Object.keys(errors).length > 0 ? Object.keys(errors) : (() => { validate(); return Object.keys(errors); })();
      
      // Re-validate to get fresh errors
      const errs: Record<string, string> = {};
      if (!form.department.trim()) errs.department = 'Обязательное поле';
      if (!form.task.trim()) errs.task = 'Обязательное поле';
      if (!form.issuerId) errs.issuerId = 'Выберите выдающего наряд-допуск';
  ...
  const handleSign = (sig: EDSSignature) => {
    if (savedPermitId) {
