...
// ── Personnel row ─────────────────────────────────────────────────────────────
function PersonRow({
  num, label, userId, userName, sig, status,
}: {
  num: string; label: string; userId?: string; userName?: string; sig?: EDSSignature; status: SigStatus;
}) {
  const u = getUser(userId);
  const name = userName || u?.name;
  const info = u ? `${u.position} · Гр.ЭБ ${u.electricalGroup}` : '';
      ...
      <div className="col-span-4 px-3 py-2.5">
        {name ? (
          <div>
            ...
            {info && <p className="text-[10px] text-gray-400">{info}</p>}
          </div>
        ) : (
...
export function WorkPermitDetail() {
  ...
  if (ctx.loading) return (
    <div className="flex items-center justify-center min-h-screen bg-white">
      <div className="text-center">
        ...
        <p className="text-slate-500 text-sm">Загрузка наряда-допуска...</p>
      </div>
    </div>
  );
  ...
  if (!permit || !currentUser) return (
    <div className="flex items-center justify-center min-h-screen bg-white">
      <div className="text-center p-8 bg-slate-50 rounded-2xl border border-slate-200">
        ...
        <p className="text-slate-900 font-medium mb-1">Наряд не найден</p>
        <p className="text-slate-500 text-sm mb-6">Возможно, он был удалён или у вас нет прав доступа</p>
        <Link to="/permits" className="px-6 py-2.5 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">
          К списку нарядов
        </Link>
      </div>
    </div>
  ...
  const isIssuer   = role === 'issuer'               && permit.issuerId === uid;
  const isDisp     = role === 'dispatcher'            && permit.dispatcherId === uid;
  const isAssist   = role === 'dispatcher_assistant'  && permit.dispatcherAssistantId === uid;
  const isAdmitter = role === 'admitter'              && permit.admitterId === uid;
  ...
  const handleEDSSign = async (modalKey: string, sig: EDSSignature) => {
    setModal(null);
    try {
      switch (modalKey) {
        case 'issuer_sign':
        case 're_sign':       await ctx.signByIssuer(permit.id, sig); break;
        case 'disp_approve':  await ctx.signByDispatcher(permit.id, sig); break;
        case 'assist_ack':    await ctx.acknowledgeByAssistant(permit.id, sig); break;
        case 'admitter_sign':
        case 'admitter_re':   await ctx.signByAdmitter(permit.id, livePartsText, sig); break;
        case 'verify_wp': {
          const vRole = isManager ? 'manager' : isObserver ? 'observer' : 'foreman';
          await ctx.approveWorkplace(permit.id, vRole, sig);
          break;
        }
        case 'brief_admitter': if (lastBriefing) await ctx.signBriefingAdmitter(permit.id, lastBriefing.id, sig); break;
        case 'brief_resp':     if (lastBriefing) await ctx.signBriefingResponsible(permit.id, lastBriefing.id, sig); break;
        case 'end_daily':      if (lastBriefing) await ctx.endDailyWork(permit.id, lastBriefing.id, new Date().toISOString(), sig); break;
        case 'foreman_close':  await ctx.initiateClosure(permit.id, closureNotify, new Date(closureDateTime).toISOString(), sig); break;
        case 'manager_close':  await ctx.signClosureManager(permit.id, sig); break;
        case 'extend_sign':    if (extendDate) await ctx.extendPermit(permit.id, new Date(extendDate).toISOString(), sig); break;
      }
      const msg = TOAST_MSGS[modalKey];
      if (msg) pushToast(msg, 'success', 'Действие успешно выполнено');
    } catch (e) {
      console.error(e);
      pushToast('Ошибка', 'danger', 'Не удалось выполнить действие');
    }
  };
                      ...
                      return (
                        <div>
                          ...
                          <div className="border border-gray-200 rounded-lg overflow-hidden">
                            ...
                            {/* Rows — core workflow */}
                            <PersonRow num="①" label="Выдающий наряд-допуск"    userId={permit.issuerId}              userName={(permit as any).issuerName}              sig={permit.issuerSignature}               status={cs(permit.issuerId, permit.issuerSignature)} />
                            <PersonRow num="②" label="Главный диспетчер"         userId={permit.dispatcherId}          userName={(permit as any).dispatcherName}          sig={permit.dispatcherSignature}           status={cs(permit.dispatcherId, permit.dispatcherSignature)} />
                            <PersonRow num="③" label="Помощник ГД"               userId={permit.dispatcherAssistantId} userName={(permit as any).dispatcherAssistantName} sig={permit.dispatcherAssistantSignature}  status={cs(permit.dispatcherAssistantId, permit.dispatcherAssistantSignature)} />
                            <PersonRow num="④" label="Допускающий"               userId={permit.admitterId}            userName={(permit as any).admitterName}            sig={permit.admitterWorkplaceSignature}    status={cs(permit.admitterId, permit.admitterWorkplaceSignature)} />

                            {/* Manager — verifier + closure */}
                            <PersonRow num="⑤" label="Отв. руководитель"
                              userId={permit.managerId}
                              userName={(permit as any).managerName}
                              sig={managerBestSig}
                            ...
                            <PersonRow num="⑥" label="Наблюдающий"
                              ...
                              userName={(permit as any).observerName}
                              sig={observerBestSig}
                            ...
                            <PersonRow num="⑦" label="Производитель работ"
                              ...
                              userName={(permit as any).foremanName}
                              sig={foremanBestSig}
