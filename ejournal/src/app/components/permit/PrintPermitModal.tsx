...
function PermitDocument({ permit }: { permit: WorkPermit }) {
  const issuerName   = permit.issuerName || getUser(permit.issuerId)?.name || '';
  const managerName  = permit.managerName || getUser(permit.managerId)?.name || '';
  const admitterName = permit.admitterName || getUser(permit.admitterId)?.name || '';
  const foremanName  = permit.foremanName || getUser(permit.foremanId)?.name || '';
  const observerName = permit.observerName || getUser(permit.observerId)?.name || '';
  const dispName     = permit.dispatcherName || getUser(permit.dispatcherId)?.name || '';
  const assistName   = permit.dispatcherAssistantName || getUser(permit.dispatcherAssistantId)?.name || '';
        ...
        <tbody>
          <tr>
            <td style={{ border: 'none', width: '50%', paddingBottom: 3 }}>
              {S('Ответственному руководителю работ', managerName, 130)}
            </td>
            <td style={{ border: 'none', paddingBottom: 3 }}>
              {S('Допускающему', admitterName, 150)}
            </td>
          </tr>
          <tr>
            <td style={{ border: 'none', paddingBottom: 3 }}>
              {S('Производителю работ', foremanName, 155)}
            </td>
            <td style={{ border: 'none', paddingBottom: 3 }}>
              {S('Наблюдающему', observerName, 160)}
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td style={{ minHeight: 24 }}>
              {permit.dispatcherSignature
                ? `${getUser(permit.dispatcherId)?.position || 'Главный диспетчер'}, ${dispName}`
                : <>&nbsp;<br />&nbsp;</>}
            </td>
            ...
            <td className="center">
              {permit.dispatcherAssistantSignature ? (
                <>
                  <span className="eds-mark">ЭЦП</span>{' '}
                  <span style={{ fontSize: '8.5pt' }}>{assistName}</span>
                  <br />
              ...
              <tr key={b.id}>
                ...
                <td className="center" style={{ fontSize: '7.5pt' }}>
                  {b.admitterSignature ? (
                    ...
                    <span style={{ fontSize: '7pt' }}>{getUser(permit.admitterId)?.shortName || b.admitterSignature.userName.split(' ').slice(0,2).join(' ')}</span></>
                  ) : ''}
                </td>
                <td className="center" style={{ fontSize: '7.5pt' }}>
            ...
            <tr>
              <td style={{ border: 'none', width: '50%' }}>
                ...
                {permit.foremanClosureSignature && (
                  <span style={{ fontSize: '8.5pt' }}> {getUser(permit.foremanId)?.shortName || foremanName}</span>
                )}
              </td>
              {permit.managerClosureSignature && (
                <td style={{ border: 'none' }}>
                  ...
                  {'  '}
                  <span style={{ fontSize: '8pt', fontStyle: 'italic' }}>(подпись)</span>
                  <span style={{ fontSize: '8.5pt' }}> {getUser(permit.managerId)?.shortName || managerName}</span>
                </td>
              )}
            </tr>
          </tbody>
